...
console.log("%c[BK DEBUG] FIX 8 SHELL-MOVER worldMap2.js GELADEN", "background:#860000;color:#fff;font-weight:bold;padding:4px 8px");
console.log("[BK DEBUG] FIX 8 worldMap2 timestamp", new Date().toISOString());
...
marker.style.bottom =
  ...
  prepareProgressMover(fromLevel, toLevel) {
    ...
    if (!shell) {
      console.error("[BK DEBUG] FIX 8 ABBRUCH: .world2-shell fehlt");
    ...
    console.log("[BK DEBUG] FIX 8 MOVER IN FESTER SHELL", {
      connected: mover.isConnected,
      parent: mover.parentElement?.className,
      viewportScrollTop: this.viewport.scrollTop,
